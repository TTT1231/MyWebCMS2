...
const rules = reactive<FormRules<RuleForm>>({
  ...
  vaildcode: [
    { required: true, message: () => t('login.placeholderPhoneCode'), trigger: 'blur' }
  ]
});
...
<template>
   <div class=" px-5 bg-white" v-show="isShow">
      <span class="font-bold text-2xl flex justify-center my-5 xl:justify-start xl:-ml-8 animate-enterx-faster">
      ...
      <el-form ref="ruleFormRef"
        ...
        >
         <el-form-item 
          prop="mobile"
         :label="$t('login.phone')" class=" animate-enterx-faster">
            <el-input v-model="ruleForm.mobile":placeholder="$t('login.placeholderPhone')" />
         </el-form-item>
         <el-form-item 
         prop="vaildcode"
         :label="$t('login.phoneCode')" class=" animate-enterx-medspeed">
            <div class=" flex">
                <el-input  type="text"
                ...
                <el-button
                  ...
                  :disabled="start"
                  class=" ml-5 w-1/3 "
                  >{{countdownTip}}</el-button
               >
            </div>
         </el-form-item>
      </el-form>
      <div class="flex flex-col gap-4" >
         <AButton type='primary' class=" animate-enterx-medspeed ease-out mt-4">{{$t('login.DefaultLogin')}}</AButton>
         <AButton type='default' class=" animate-enterx-medspeed ease-out" @click="handleBack">{{$t('login.back')}}</AButton>
